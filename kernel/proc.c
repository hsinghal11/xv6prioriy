...
// For context switch counting and printing "330"
int contextSwitches = 0;
struct spinlock cswitch_lock;
...
procinit(void)
{
  ...
  initlock(&cswitch_lock, "cswitch_lock");
  for(p = proc; p < &proc[NPROC]; p++) {
...
userinit(void)
{
  ...
  p->state = RUNNABLE;
  p->priority = 20; // Highest priority for init process
...
// Simple LCG (Linear Congruential Generator) for pseudorandom numbers
static uint
random(uint limit)
{
  static uint lfsr = 0x12345;  // Initial seed
...
scheduler(void)
{
  ...
  for(;;){
    ...
    uint winning_ticket = random(total_tickets);
...
// Get the priority of a process given its pid.
// Returns the priority (0-20) or -1 if the process doesn't exist.
int
getPriority(int pid)
{
  struct proc *p;
...
// Only the parent process can set the priority.
// Returns 0 on success, -1 on error (invalid pid, not parent, or invalid priority).
int
...
// Set the number of tickets for a process given its pid.
// Returns 0 on success, -1 on error (invalid pid or tickets out of range).
int
setTickets(int pid, int tickets)
{
  ...
  if(tickets < 1 || tickets > 100)
    return -1;  // Tickets must be between 1 and 100
